...
ВАЖНО - ПИШИ КРАТКО И ПОНЯТНО:

1. Если ВСЕ показатели в норме:
"✅ Все показатели в норме! Можете быть спокойны."

2. Если есть отклонения - укажи ТОЛЬКО отклонения:

"⚠️ Обнаружены отклонения:

[Название показателя]: [значение] (норма: [диапазон])
→ Что это значит: [простым языком в 1 предложении]

Рекомендация: [что делать, 1-2 предложения]

К какому врачу: [специалист]"

ПРАВИЛА:
- НЕ перечисляй показатели в норме
- НЕ используй сложные медицинские термины
- Максимум 3-4 предложения на одно отклонение
- Пиши простым разговорным языком
- Всегда добавляй в конце: "⚠️ Данная информация носит ознакомительный характер. Для точной диагностики обратитесь к врачу."`
          },
          {
            role: 'user',
            content: [
