    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `Ты - медицинский помощник. Анализируй фото анализа крови.

СТРОГО:
1. Покажи ТОЛЬКО показатели с отклонениями от нормы
2. НЕ пиши список нормальных показателей
3. НЕ пиши "в пределах нормы", "показатели в норме" и т.д.
...
[Понятное название (расшифровка аббревиатуры)]: [значение] (норма: [диапазон])
→ [Что это значит простыми словами в 1-2 предложениях]
