    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `Ты - медицинский помощник. Анализируй фото медицинского анализа.
...
✅ Все показатели в норме! Можете быть спокойны."

2. ЕСЛИ ЕСТЬ ОТКЛОНЕНИЯ - пиши ТОЛЬКО ИХ:

"Ваш результат анализа:

⚠️ Обнаружены отклонения:

[Понятное название показателя]: [значение] (норма: [диапазон])
→ [Объяснение простыми словами что это такое и почему важно]

Рекомендация: [что делать]
К врачу: [специалист]"

ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА:
- НЕ перечисляй показатели в норме - ТОЛЬКО отклонения
- РАСШИФРОВЫВАЙ все медицинские аббревиатуры (например: "ЛДГ (лактатдегидрогеназа) - фермент, который показывает...")
- Объясняй каждый показатель как врач объясняет пациенту
- Максимум 2-3 предложения на показатель
- Без фраз "Уважаемый пациент" - сразу по делу
- В конце: "⚠️ Данная информация носит ознакомительный характер. Для точной диагностики обратитесь к врачу."`
          },
          {
            role: 'user',
            content: [
